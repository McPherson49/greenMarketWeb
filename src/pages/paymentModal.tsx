...
export const FreePlanSuccessModal: React.FC<FreePlanSuccessModalProps> = ({ isOpen, onClose }) => {
  const [mounted, setMounted] = useState(false);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute -top-10 left-1/2 -translate-x-1/2 w-40 h-40 bg-green-100/20 rounded-full blur-3xl"></div>
        <div className="absolute -bottom-10 right-1/2 translate-x-1/2 w-32 h-32 bg-green-100/10 rounded-full blur-3xl"></div>

        <div className="relative text-center pt-20 pb-14 px-6">
          <div className="flex justify-center mb-6">
            <div className="relative w-28 h-28 bg-linear-to-br from-green-100/30 to-emerald-100/30 rounded-3xl flex items-center justify-center shadow-lg border border-green-200/50 animate-glow">
              <FaCheckCircle className="text-green-600 text-7xl drop-shadow-xl" />
            </div>
          </div>
          ...
          <p className="text-gray-700 text-lg mb-4 leading-relaxed max-w-sm mx-auto">
            Your ad has been published with the <span className="font-semibold text-green-600">Free Plan</span>.
          </p>

          <p className="text-gray-600 text-sm mb-10 leading-relaxed max-w-sm mx-auto">
            Your ad is now live and visible to potential buyers. You can upgrade to a premium plan anytime to get more visibility and features.
          </p>

          <button
            onClick={onClose}
            className="w-full bg-linear-to-r from-green-500 to-emerald-600 hover:opacity-90 text-white font-semibold py-4 px-8 rounded-2xl transition-all duration-200 shadow-xl hover:shadow-2xl active:scale-95 relative overflow-hidden"
          >
            <span className="relative z-10">View My Ad</span>
            <div className="absolute inset-0 bg-linear-to-r from-transparent via-white/20 to-transparent -skew-x-12 animate-shimmerSlow"></div>
          </button>
        </div>
      </div>
