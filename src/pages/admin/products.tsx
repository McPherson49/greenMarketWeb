...
import { FiSearch, FiX, FiTrash2, FiTrendingUp, FiDownload } from "react-icons/fi";
...
export default function ProductManagement() {
  ...
  const [pagination,       setPagination]       = useState({
    current_page: 1, last_page: 1, per_page: 8, total: 0, from: 1, to: 1,
  });
  ...
  const transformToUIProduct = (product: Product): UIProduct => {
    ...
    return {
      id: product.id, category_id: product.category_id, old_id: product.old_id,
      user_id: product.user_id, title: product.title, slug: product.slug,
      tags: product.tags, description: product.description, state: product.state,
      local: product.local, nearest: product.nearest, plan_id: product.plan_id,
      price: product.price, use_escrow: product.use_escrow, status: product.status,
      images: product.images, price_range: product.price_range, meta: product.meta,
      properties: product.properties, created_at: product.created_at,
      updated_at: product.updated_at, views: product.views, user: product.user,
      social: product.social, business: product.business,
      purchased_ad: product.purchased_ad, thumbnail: product.thumbnail,
      keyword: product.keyword, address: product.address, phone: product.phone,
      chat_id: product.chat_id, reviews: product.reviews,
      subscription: product.subscription, icon: product.icon, sub: product.sub,
      uiRating, uiReviewCount, originalPrice,
  ...
  const fetchProducts = useCallback(async (page: number = 1, search?: string) => {
    setLoading(true);
    try {
      const params: ProductsParams = { page, per_page: itemsPerPage, search: search || undefined };
      const response = await ProductService.getProducts(params);
      setProducts(response.data.map(transformToUIProduct));
      setPagination({
        current_page: response.current_page, last_page: response.last_page,
        per_page: response.per_page, total: response.total,
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      if (selectedProduct?.id === productId) setSelectedProduct({ ...selectedProduct, status: newStatus });
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-blue-50 to-green-50">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-[#39B54A]">Product Management</h1>
        ...
        <div className="mb-8 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 bg-white rounded-2xl p-6 border border-gray-100">
          ...
          <div className="flex items-center gap-3 w-full lg:w-auto">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-3 text-sm font-medium text-white bg-green-600 hover:bg-green-700 disabled:bg-green-300 disabled:cursor-not-allowed rounded-xl transition-colors whitespace-nowrap"
                ...
                <div key={product.id} className="bg-white rounded-2xl hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 group">
                  <div className="relative overflow-hidden h-56 bg-linear-to-br from-gray-100 to-gray-50">
                    ...
                    {product.originalPrice && (
                      <div className="absolute top-3 left-3 bg-red-500 text-white px-3 py-1 rounded-full text-xs font-bold shadow-md">SALE</div>
                    )}
                    <div className="absolute bottom-3 left-3">
                      <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                        product.status === "publish" ? "bg-green-100 text-green-700"
                        : product.status === "pending" ? "bg-yellow-100 text-yellow-700"
                        : product.status === "draft" ? "bg-blue-100 text-blue-700"
                  ...
                  <div className="p-5">
                    <h3 className="text-sm font-semibold mb-3 h-10 line-clamp-2 text-gray-800 group-hover:text-green-600 transition">{product.title}</h3>
                    ...
                    <div className="text-xs text-gray-500 mb-3">
                      <span className="flex items-center gap-1">📍 {product.state}, {product.local}</span>
                      <span className="flex items-center gap-1 mt-1">👤 {product.user?.name || "Unknown Seller"}</span>
                    </div>
                    <button
                      onClick={() => setSelectedProduct(product)}
                      className="w-full bg-[#39B54A] text-white py-3 rounded-lg transition-all duration-300 flex items-center justify-center gap-2 font-medium shadow-md hover:shadow-lg hover:bg-green-600"
                    >
            ...
            <div className="flex justify-center items-center gap-3">
              ...
              {totalPages > 5 && currentPage < totalPages - 2 && (
                <>
                  ...
                  <button onClick={() => setCurrentPage(totalPages)} disabled={loading} className="px-5 py-2 rounded-lg bg-white text-gray-700 hover:bg-gray-50 hover:shadow-md border border-gray-200 transition-all duration-300 font-medium">
                    {totalPages}
                  </button>
                </>
              )}
        ...
        {!loading && products.length === 0 && (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500">{searchTerm ? "No products match your search." : "No products available at the moment."}</p>
            {searchTerm && (
              <button onClick={() => setSearchTerm("")} className="mt-4 text-green-600 hover:text-green-700 font-medium">
                Clear search
              </button>
            )}
          </div>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white rounded-3xl max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl">
              <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b border-gray-100 p-6 flex justify-between items-center z-10 rounded-t-3xl">
                ...
                <button onClick={() => setSelectedProduct(null)} className="text-gray-400 hover:text-gray-600 transition p-2 hover:bg-gray-100 rounded-full">
                  <FiX size={24} />
                </button>
              </div>
              ...
              <div className="p-6 space-y-6">
                <div className="relative overflow-hidden rounded-2xl h-72 bg-linear-to-br from-gray-100 to-gray-50 shadow-inner">
                  ...
                  <div className="absolute bottom-4 left-4">
                    <span className={`inline-flex px-3 py-1 rounded-full text-sm font-medium ${
                      selectedProduct.status === "publish" ? "bg-green-100 text-green-700"
                      : selectedProduct.status === "pending" ? "bg-yellow-100 text-yellow-700"
                      : selectedProduct.status === "draft" ? "bg-blue-100 text-blue-700"
                      : "bg-gray-100 text-gray-700"
                    }`}>
                ...
                <div>
                  ...
                  <div className="flex items-baseline gap-3 mb-6">
                    {selectedProduct.originalPrice && (
                      <span className="text-gray-400 line-through text-xl">₦{formatPrice(selectedProduct.originalPrice)}</span>
                    )}
                      ...
                      <div className="mt-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedProduct.tags.map((tag, index) => (
                            <span key={index} className="bg-white px-3 py-1 rounded-full text-xs text-gray-600 border border-gray-200">{tag}</span>
                          ))}
                        </div>
                      </div>
                    )}
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    ...
                    className="flex items-center justify-center gap-3 bg-blue-500 hover:bg-blue-600 text-white px-6 py-4 rounded-xl transition-all duration-300 shadow-md hover:shadow-lg font-medium"
                  >
                    <FiTrendingUp size={20} /> Boost Ad
                  </button>
                  <button
                    onClick={() => handleDelete(selectedProduct.id)}
                    className="flex items-center justify-center gap-3 bg-red-500 hover:bg-red-600 text-white px-6 py-4 rounded-xl transition-all duration-300 shadow-md hover:shadow-lg font-medium"
                  >
                ...
                <div className="bg-gray-50 rounded-2xl p-5 border border-gray-200">
                  <h4 className="font-semibold text-gray-800 mb-4">Change Status</h4>
                  <div className="grid grid-cols-2 gap-3">
                    {(["pending", "publish", "draft", "trash"] as const).map((status) => (
                      <button
                        key={status}
                        onClick={() => handleStatusChange(selectedProduct.id, status)}
                      ...
                      >
                        {status === "pending" ? "⏳ Pending" : status === "publish" ? "✓ Publish" : status === "draft" ? "📝 Draft" : "🗑️ Trash"}
                      </button>
                    ))}
                  </div>
                </div>
