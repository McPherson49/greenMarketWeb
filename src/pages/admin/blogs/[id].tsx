  ...
  useEffect(() => {
    ...
    const fetchBlog = async () => {
      ...
      try {
        ...
        console.error(err);
        setFetchError("Failed to load blog post. Please go back and try again.");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (fetchError || !formData) {
    return (
      <div className="text-center py-32">
        <p className="text-red-500 mb-4">{fetchError ?? "Blog post not found."}</p>
        <Link href="/admin/blogs" className="text-green-600 underline">← Back to blogs</Link>
      </div>
    );
  }
        ...
        <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
              ...
              <p className="text-sm text-gray-500 mb-3">The main body of the blog post.</p>
              <textarea
                ...
                {previewImage ? (
                  <div className="relative max-w-4xl mx-auto">
                    <div className="relative aspect-[16/9] rounded-xl overflow-hidden shadow-lg">
                      <Image src={previewImage} alt="Featured" fill className="object-cover" />
                    </div>
                    ...
                    {formData.image instanceof File && (
                      <p className="text-xs text-gray-500 mt-2">New: {formData.image.name}</p>
                    )}
                    {typeof formData.image === "string" && (
                      <label className="cursor-pointer block mt-3">
                        <input type="file" accept="image/*" onChange={handleImageChange} className="hidden" />
                        <span className="text-sm text-green-600 underline">Replace image</span>
                      </label>
                    )}
                  </div>
                ) : (
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-8 shadow-sm">
              ...
              <p className="text-sm text-gray-500 mb-6">A secondary content section with its own title and body.</p>
            ...
            <div className="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm">
              ...
              <div className="space-y-3">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="radio" name="status" checked={formData.status === "draft"} onChange={() => setFormData((p) => p ? { ...p, status: "draft" } : p)} className="w-4 h-4 text-green-600" />
                  <div>
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="radio" name="status" checked={formData.status === "published"} onChange={() => setFormData((p) => p ? { ...p, status: "published" } : p)} className="w-4 h-4 text-green-600" />
                  <div>
