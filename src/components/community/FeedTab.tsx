...
import { Loader2, AlertCircle, ImagePlus, X, Send, Sprout } from "lucide-react";
...
/** Safely extract ApiPost[] from any response shape the API might return */
          ...
          {!showExpanded ? (
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 bg-green-100 rounded-full flex items-center justify-center text-sm font-bold text-green-700 shrink-0">
                {currentUserId ? currentUserId.toString().charAt(0) : "U"}
              </div>
              <button
      ...
      {!isLoading && !error && posts.length === 0 && (
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          ...
          {isMember && token ? (
            ...
            <p className="text-gray-400 text-sm">Nothing has been posted here yet. Check back soon.</p>
          )}
        </div>
      )}
